...
await the whole chain before ending the response. The chain runs on **every**
request (not only matched routes), so a middleware that responds *without*
calling `अग्रे()` short-circuits before route matching — the hook that makes
static-file serving and CORS preflight work.
...
### The web middleware stack — cookies, sessions, static, CORS

On top of the router sit the pieces a real web app needs, each a Devabhāṣā
middleware:

- **`std/कुकी`** — `पठ(शीर्षाणि)` parses the Cookie header into a `कोष`;
  `सज्जय(नाम, मूल्यम्, विकल्पाः?)` builds a `Set-Cookie` string (Path/Max-Age/
  SameSite/HttpOnly/Secure).
- **`std/सत्र`** — `सत्रमध्यगः(कुञ्जी)` is **signed-cookie sessions**, tying Arc D
  to Arc E: the session is a `पत्रम्` (signed token) kept in a cookie, so it is
  tamper-evident and needs no server-side store. The middleware verifies the
  cookie into `अनुरोधः.सत्रम्` (the payload, or `शून्यम्`) and adds
  `प्रत्युत्तरम्.सत्रस्थापय(दत्तम्)` / `सत्रनाशः()` to log in / out.
- **`std/जालमध्यगाः`** — `स्थैतिक(मूलम्)` serves text assets under a directory
  (with a path-traversal guard and content-type by extension); `कोर्स(विकल्पाः?)`
  sets CORS headers and answers an `OPTIONS` preflight with `204`.

```
ऐप.उपयुज्(कोर्स())।                    # CORS on every response
ऐप.उपयुज्(सत्रमध्यगः(कुञ्जी))।         # अ.सत्रम् + प्र.सत्रस्थापय/सत्रनाशः
ऐप.उपयुज्(स्थैतिक("./सार्वजनिक"))।      # static text assets, else fall through
ऐप.प्राप्("/अहम्", कार्य(अ, प्र){ प्र.प्रेषय_जेसन(अ.सत्रम् अथवा कोष{})। })।
```

`examples/जालसेवा.deva` is a complete session-based login service — **`कोर्स`,
`सत्रमध्यगः`, `स्थैतिक`, `गूढपदम्` password auth, `पर्यावरण` config, and `प्रलेख`
logging** on one router — a working web app in one Sanskrit source with no
external dependencies. All of it is verified end-to-end over real HTTP in
`test/web-middleware.test.js`.
